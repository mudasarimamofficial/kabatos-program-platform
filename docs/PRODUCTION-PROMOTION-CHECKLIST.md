...
The following 8 canonical migrations are audited, verified on DEV (`finbvtwjddrmbuuuyeni`), and queued for production execution upon authorization:
...
- [ ] `NEXT_PUBLIC_APP_URL` = Production Custom Domain (e.g., `https://program.comprex.com`)
...
- [ ] `COMPREX_STRIPE_LIVE_PRICE_ID` = Approved Client Stripe LIVE Price ID
